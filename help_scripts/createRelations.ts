...
(async function () {
  ...
  for (const row of plainData) {
    for (let i = 1; i <= 13; i++) {
      ...
      const currendGrerDateFromKokizov = new Date(
        new Date(new Date(row[`m${i}dat`]).getTime() + 1000 * 60 * 60 * 3)
          .toISOString()
          .split('T')[0],
      );
      ...
      for (let j = 0; j < Number(row[`m${i}dsk`]); j++) {
        ...
        const incrementedDate = addDay(
          new Date(`${gregDate.year}-${gregDate.month}-${gregDate.day}`),
        );
